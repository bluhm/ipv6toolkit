...
int print_ni_addr(struct iface_data *idata, const u_char *pktdata, struct pcap_pkthdr *pkthdr) {
    ...
    printf("\tFlags: %s%s%s%s%s%s%s\n", ((flags & NI_NODEADDR_FLAG_GLOBAL) ? "G" : ""),
           ((flags & NI_NODEADDR_FLAG_SITELOCAL) ? "S" : ""), ((flags & NI_NODEADDR_FLAG_LINKLOCAL) ? "L" : ""),
           ((flags & NI_NODEADDR_FLAG_COMPAT) ? "C" : ""), ((flags & NI_NODEADDR_FLAG_ALL) ? "A" : ""),
           ((flags & NI_NODEADDR_FLAG_TRUNCATE) ? "T" : ""), ((!flags) ? "none" : ""));
