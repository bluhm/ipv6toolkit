...
unsigned char buffer[FRAG_BUFFER_SIZE];
...
int main(int argc, char **argv) {
    ...
    const char shortopts[] = "i:s:d:A:u:U:H:y:c:t:r:x:moaqp:E:eP:R:M:N:S:D:f:F:w:W:lz:Lj:k:J:K:b:g:B:G:vh";
    char option;
    ...
    while ((r = getopt_long(argc, argv, shortopts, longopts, NULL)) != -1) {
        ...
        case 'p': /* "Preference" bits */
            ...
            else if (strncmp(optarg, "resvd", strlen("resvd")) == 0 ||
                     strncmp(optarg, "reserved", strlen("reserved")) == 0 || strncmp(optarg, "r", strlen("r")) == 0) {
        ...
        case 'R': /* Route Information option */
            ...
            else if (strncmp(rpref, "resvd", strlen("resvd")) == 0 ||
                     strncmp(rpref, "reserved", strlen("reserved")) == 0 || strncmp(rpref, "r", strlen("r")) == 0) {
    ...
    do {
        ...
        do {
            ...
            while (prefixes < nprefixes &&
                   (((ptr + sizeof(struct nd_opt_prefix_info)) - v6buffer) <= idata->max_packet_size)) {
            ...
            while (routes < nroutes &&
                   (((ptr + sizeof(struct nd_opt_route_info_l)) - v6buffer) <= idata->max_packet_size)) {
            ...
            if (!floodd_f) {
                while (dnsopts < nrdnss &&
                       (((ptr + sizeof(struct nd_opt_rdnss_l) + nrdnssopt[dnsopts] * sizeof(struct in6_addr)) -
                         v6buffer) <= idata->max_packet_size)) {

                    dnsopt = (struct nd_opt_rdnss_l *)ptr;
                    dnsopt->nd_opt_rdnss_type = ND_OPT_RDNSS;
                    dnsopt->nd_opt_rdnss_len =
                        (sizeof(struct nd_opt_rdnss_l) + nrdnssopt[dnsopts] * sizeof(struct in6_addr)) / 8;
                ...
                while (dnsopts < nrdnss) {
                    ...
                    if (smaxaddrs > 0) {
                        ...
                        dnsopt->nd_opt_rdnss_len = (sizeof(struct nd_opt_rdnss_l) + i * sizeof(struct in6_addr)) / 8;
            ...
            (linkaddrs > nlinkaddr || mtus < nmtu || prefixes < nprefixes || routes < nroutes || dnsopts < nrdnss) &&
            newdata_f);
        sources++;
...
void print_attack_info(struct iface_data *idata) {
    ...
    printf("Cur Hop Limit: %u   Preference: %s   Flags: %s%s%s%s%s   Router Lifetime: %u\n", curhop,
           routestr[preference], ((managed_f) ? "M" : ""), ((other_f) ? "O" : ""), ((home_f) ? "H" : ""),
           ((proxy_f) ? "P" : ""), ((!managed_f && !other_f && !home_f && !proxy_f) ? "none" : ""), lifetime);
    ...
    if (!floodp_f) {
        for (i = 0; i < nprefixes; i++) {
            ...
            printf("Prefix: %s/%u\tFlags: %s%s%s%s   ", pprefix, prefixlen[i],
                   ...
                   ((prefixflags[i] & ND_OPT_PI_FLAG_RADDR) ? "R" : ""), ((!prefixflags[i]) ? "none" : ""));
        ...
        printf("Prefix: (randomized)/%u%s   Flags: %s%s%s%s   ", prefixlen[0], (prefopt_f ? "" : " (default)"),
               ...
               ((prefixflags[0] & ND_OPT_PI_FLAG_RADDR) ? "R" : ""), ((!prefixflags[0]) ? "none" : ""));
